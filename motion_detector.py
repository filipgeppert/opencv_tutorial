...
from utils import normalize_image, display_image
...
def main():
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            _, points = draw_lanes(img=frame, lines=lanes, height=h, height_threshold=height_threshold)
            ...
            if is_first_frame:
                ...
                mask_roi = mask_gray[-height_threshold:, roi_start[0]:roi_end[0]]
                frame_difference_masked = cv2.bitwise_and(src1=frame_difference, src2=mask_roi)
                ...
                if contours_sorted:
                    ...
                    if area > 8_000:
                        ...
                        # Draw bounding box
                        cv2.rectangle(frame_color_roi, (x, y-50), (x + w, y + h), (0, 255, 0), 2)
                    else:
                        pass
